...
function DetalhesEmprestimo({ id_emprestimo }: DetalhesEmprestimoProps): JSX.Element {
    ...
    useEffect(() => {
        ...
        if (isNaN(id_emprestimo) || id_emprestimo <= 0) {
            setError("ID do empréstimo inválido.");
            setLoading(false);
            return;
        }
    ...
    return (
        <main className="bg-gray-200 flex-1 py-6 sm:py-10 px-4 overflow-y-auto">
            <Card title={`Empréstimo #${emprestimo.id_emprestimo || "N/A"}`} className="shadow-lg animate-fade-in transition-all duration-300 w-full max-w-4xl p-4 sm:p-6 md:p-8 mx-auto font-bold text-xl">
                <div className="flex flex-col gap-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-2">
                        ...
                        <div className="flex flex-col gap-4">
                            <h3 className="text-lg font-semibold text-primary-700 flex items-center gap-2">
                                <i className="pi pi-user text-blue-500"></i> Aluno
                            </h3>
                            <div className="flex flex-col gap-3 ml-1 border-l-2 border-blue-50 relative pl-4">
                                <div className="flex flex-col">
                                    ...
                                    <span className="text-gray-700 font-medium">{emprestimo.aluno.nome || "Não informado"} {emprestimo.aluno.sobrenome || ""}</span>
                                </div>
                                <div className="flex flex-col">
                        ...
                        <div className="flex flex-col gap-4">
                            <h3 className="text-lg font-semibold text-primary-700 flex items-center gap-2">
                                <i className="pi pi-book text-green-500"></i> Livro
                            </h3>
                    ...
                    <div className="flex flex-col gap-4">
                        <h3 className="text-lg font-semibold text-primary-700 flex items-center gap-2">
                            <i className="pi pi-calendar text-orange-500"></i> Datas
                        </h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 ml-1 border-l-2 border-orange-50 relative pl-4">
                            <div className="flex flex-col">
                                <span className="text-xs uppercase text-gray-400 font-bold tracking-wider">Data de Empréstimo</span>
                                <span className="text-gray-700 font-medium">{emprestimo.data_emprestimo ? new Date(emprestimo.data_emprestimo).toLocaleDateString('pt-BR') : "Não informado"}</span>
                            </div>
                            <div className="flex flex-col">
            ...
            <div className="w-full max-w-4xl mx-auto mt-6 sm:mt-8">
                <button
                    ...
                    onClick={() => emprestimo.id_emprestimo && navigate(`/atualizar/emprestimo/${emprestimo.id_emprestimo}`)}
                >
                    Editar Empréstimo
                </button>
                <button
