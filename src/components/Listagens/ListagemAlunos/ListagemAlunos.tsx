...
function ListagemAlunos(): JSX.Element {
  ...
  const handleRemoverAluno = async (id_aluno: number) => {
    ...
    if (confirmar) {
      try {
        // chama a função removerAluno passando o ID e salva a resposta
        const sucesso = await AlunoRequests.removerAluno(id_aluno);
        // caso o aluno seja removido, exibimos um alerta de sucesso ao usuário
        if (sucesso) {
  ...
  return (
    <main className="bg-gray-200 flex-1 flex flex-col px-4 sm:px-6 md:px-10 py-6 md:py-10 overflow-hidden">
      {" "}
      {/* overflow-hidden no main para conter o scroll interno */}
          ...
          <table className="table-auto w-full border-collapse text-xs sm:text-sm md:text-base">
            <thead className="bg-slate-700 sticky top-0 z-10 shadow-sm">
              <tr>
                ...
                <th className="border-b border-slate-600 text-white p-3 md:p-4 hidden lg:table-cell text-left">
                  Telefone
                </th>
              ...
              {currentAlunos && currentAlunos.length > 0 ? (
                currentAlunos.map((aluno) => (
                  ...
                  >
                    ...
                    <td className="p-3 md:p-4 hidden lg:table-cell text-slate-600">
                      {aluno.celular}
                    </td>
                    <td className="p-2 md:p-4">
                      <div className="flex flex-col sm:flex-row items-center justify-center gap-1 md:gap-2">
                        ...
                        <button
                          ...
                          onClick={() =>
                            aluno.id_aluno && handleRemoverAluno(aluno.id_aluno)
                          }
                        >
                          Deletar
                        </button>
          ...
          <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
            <div>
              <p className="text-sm text-slate-700">
                Mostrando{" "}
                <span className="font-semibold">{indexOfFirstRow + 1}</span> até{" "}
                <span className="font-semibold">
                  {Math.min(indexOfLastRow, alunos.length)}
                </span>{" "}
                de <span className="font-semibold">{alunos.length}</span>{" "}
                resultados
              </p>
            </div>
            <div>
